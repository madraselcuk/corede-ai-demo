...
const FaqHeroContent = () => {
  ...
  const scrollToQuestions = () => {
    const questionsElement = document.getElementById('faq-questions')
    if (questionsElement) {
      questionsElement.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
  }
  ...
  return (
    <div className="flex flex-col items-center py-14 md:py-20">
      ...
      <div className="flex justify-center">
        <GradientBorderButton text={t('button')} className="w-fit" onClick={scrollToQuestions} />
      </div>
    </div>
  )
}
